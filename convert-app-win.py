...
def dcm2img():
    input_folder = directory1Select.folder_path.replace('/','\\')
    #print(input_folder)#get input path
    output_folder = directory2Select.folder_path.replace('/','\\') #get output path
    ...
    for file in item_list:
        if has_file_allowed_extension(file, IMG_EXTENSIONS):  ## except only cdm file 
            ...
            mkdir( output_folder  + file.rpartition('\\')[0]) # ok
            output_path = output_folder + file.replace('.dcm', '.png')
            try: 
...
def dcm2img():
    ...
    ## get path folder (for windows)
    input_folder  = directory1Select.folder_path.replace('/','\\')
    output_folder = directory2Select.folder_path.replace('/','\\')
    ## get all items
    item_list = get_all_items(input_folder)
    ## using multiprocessing to convert dicom files to images
